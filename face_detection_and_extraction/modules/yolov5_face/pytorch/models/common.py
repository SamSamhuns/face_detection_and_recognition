        ...
        for i, im in enumerate(imgs):
            if isinstance(im, str):  # filename or uri
                im = Image.open(requests.get(
                    im, stream=True).raw if im.startswith('http') else im)  # open
...
class Detections:
    ...
    def display(self, pprint=False, show=False, save=False, render=False):
        ...
        for i, (img, pred) in enumerate(zip(self.imgs, self.pred)):
            string = f'Image {i + 1}/{len(self.pred)}: {img.shape[0]}x{img.shape[1]} '
            if pred is not None:
                for c in pred[:, -1].unique():
                    n = (pred[:, -1] == c).sum()  # detections per class
                    string += f'{n} {self.names[int(c)]}s, '  # add to string
                if show or save or render:
                    img = Image.fromarray(img.astype(np.uint8)) if isinstance(
                        img, np.ndarray) else img  # from np
                    for *box, conf, cls in pred:  # xyxy, confidence, class
                        # string += '%s %.2f, ' % (names[int(cls)], conf)  # label
