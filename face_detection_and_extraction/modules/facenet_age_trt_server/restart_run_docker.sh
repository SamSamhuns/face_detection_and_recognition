...
echo "Stopping docker container 'facenet_age_ctn' if it is running"
docker stop facenet_age_ctn || true
docker rm facenet_age_ctn || true

echo "Running docker with exposed triton-server GRPC port: $grpc"
docker run --rm \
      --shm-size=1g --ulimit memlock=-1 --ulimit stack=67108864 \
      --gpus device="0" \
      --name facenet_age_ctn \
      -p "$grpc":8081 \
      facenet_age_net:latest
