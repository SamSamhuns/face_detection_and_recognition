...
def random_perspective(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    S = np.eye(3)
    S[0, 1] = math.tan(
        system_random.uniform(-shear, shear) * math.pi / 180)  # x shear (deg)
    S[1, 0] = math.tan(
        system_random.uniform(-shear, shear) * math.pi / 180)  # y shear (deg)

    # Translation
    T = np.eye(3)
    T[0, 2] = system_random.uniform(
        0.5 - translate, 0.5 + translate) * width  # x translation (pixels)
    T[1, 2] = system_random.uniform(
