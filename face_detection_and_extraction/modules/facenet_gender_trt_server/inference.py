...
sys.path.append(".")
from modules.facenet_gender_trt_server.triton_utils import FlagConfig, get_client_and_model_metadata_config
from modules.facenet_gender_trt_server.triton_utils import parse_model_grpc, get_inference_responses
...
class TritonServerInferenceSession(object):
    ...
    def __init__(self,
                 model_name="facenet_gender",
                 container_name="facenet_gender_ctn",
        ...
        docker.build("modules/facenet_gender_trt_server/",
                     tags="facenet_gender_net:latest")
        ...
        self.container = docker.run(image="facenet_gender_net:latest", name=container_name,
    ...
    def inference_trt_model_facenet(inf_sess, cv2_image, model_in_size, **kwargs):
        ...
        image_data = np.expand_dims(rsz_image, axis=0)
        if len(image_data) == 0:
        ...
        features = detections.as_numpy("predictions")
...
if __name__ == "__main__":
    ...
    inf.container.kill()
    print("facenet gender out shape", out.shape, type(out))
