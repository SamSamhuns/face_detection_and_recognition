       ...
       # self.training |= self.export
        if self.export:
            for i in range(self.nl):
                x[i] = self.m[i](x[i])
                # x(bs,48,20,20) to x(bs,3,20,20,16)
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                #y[..., 5:15] = y[..., 5:15] * 8 - 4
                y[..., 5:7] = y[..., 5:7] * self.anchor_grid[i] + \
                    self.grid[i].to(x[i].device) * \
                    self.stride[i]  # landmark x1 y1
                y[..., 7:9] = y[..., 7:9] * self.anchor_grid[i] + \
                    self.grid[i].to(x[i].device) * \
                    self.stride[i]  # landmark x2 y2
                y[..., 9:11] = y[..., 9:11] * self.anchor_grid[i] + \
                    self.grid[i].to(x[i].device) * \
                    self.stride[i]  # landmark x3 y3
                y[..., 11:13] = y[..., 11:13] * self.anchor_grid[i] + \
                    self.grid[i].to(x[i].device) * \
                    self.stride[i]  # landmark x4 y4
                y[..., 13:15] = y[..., 13:15] * self.anchor_grid[i] + \
                    self.grid[i].to(x[i].device) * \
...
class Model(nn.Module):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None):
        ...
        if isinstance(cfg, dict):
            ...
            with open(cfg) as f:
                self.yaml = yaml.safe_load(f, Loader=yaml.FullLoader)  # model dict
