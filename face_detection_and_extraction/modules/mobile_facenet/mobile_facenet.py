...
# test transform for mobile facenet embedding extraction
...
if __name__ == "__main__":
    ...
    # network input shape is [batch, 3, 112, 112], output shape is [batch, 512]
    input = torch.Tensor(2, 3, 112, 112).to(device)
    model = MobileFaceNet(512).to(device)  # embeding size is 512 (feature vector)
    model.load_state_dict(torch.load(
        'weights/mobile_facenet/MobileFace_Net', map_location=lambda storage, loc: storage))
    model.eval()

    print(f"torch in shape: {input.shape}")
